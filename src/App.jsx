        ...
        <form>
          <Flex gap={'5px'} className="addItem">
            ...
            <Button 
              width={'150px'}
              type="submit"
              onClick={(e) => {e.preventDefault(); addItem(toDoInput, toDoDate)}}
            >
              Add item
            </Button>
          </Flex>
                ...
                <Card key={key} padding={'10px'} marginBottom={'5px'}>
                  <Flex flexWrap={'wrap'} justifyContent={'space-between'}>
                    <Flex flexDirection={'column'}>
                      <Checkbox 
                        onChange={ e => itemStatus(todoItem.item, todoItem.date, e.target.checked, toDoList) } 
                        isChecked={false}
                      >
                        {todoItem.item}
                      </Checkbox>
                      <small style={{'padding-left':'23px', 'font-size': '.6875rem'}}>to do until {todoItem.date}</small>
                    </Flex>
                    <Button type="button" onClick={() => removeItem(todoItem.item, toDoList)} _hover={{ background: 'red.500' }}>
                      <DeleteIcon />
                    </Button>
                  </Flex>
                </Card>
                ...
                return (
                  <Card key={key} padding={'10px'} marginBottom={'5px'}>
                    <Flex justifyContent={'space-between'}>
                      <Flex flexDirection={'column'}>
                        <Checkbox 
                          onChange={ e => itemStatus(todoItem.item, todoItem.date, e.target.checked, toDoListChecked) }
                          isChecked={true}
                        >
                          <s style={{'opacity':'.5'}}><i>{todoItem.item}</i></s>
                        </Checkbox>
                      <small style={{'padding-left':'23px', 'opacity':'.5', 'font-size': '.6875rem'}}>to do until {todoItem.date}</small>
                      </Flex>
                      <Button type="button" onClick={() => removeItem(todoItem.item, toDoListChecked)} _hover={{ background: 'red.500' }}>
                        <DeleteIcon />
                      </Button>
                    </Flex>
                  </Card>
